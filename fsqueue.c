...
static int _lock(int dirfd, char *file)
{
	int status = FSQ_OK;

	int fd = openat(
		dirfd, file, O_CREAT | O_WRONLY | O_EXCL, 0644);
	if(fd < 0) {
		if(errno == EEXIST)
...
static void _unlock(int dirfd, char *file)
{
...
int fsq_open(struct fsq *q, const char *path, int flags)
{
	...
	if(flags & FSQ_PRODUCE) {
		if((status = _lock(q->dirfd, WR_LOCK_NAME)))
			goto error;
		q->flags |= FSQ_PRODUCE;
	}

	if(flags & FSQ_CONSUME) {
		if((status = _lock(q->dirfd, RD_LOCK_NAME)))
			goto error;
		q->flags |= FSQ_CONSUME;
	}
...
void fsq_close(struct fsq *q)
{
	...
	if(q->flags & FSQ_PRODUCE)
		_unlock(q->dirfd, WR_LOCK_NAME);
	if(q->flags & FSQ_CONSUME)
		_unlock(q->dirfd, RD_LOCK_NAME);
	if(q->data_dirfd >= 0)
		close(q->data_dirfd);
	if(q->dirfd >= 0)
		close(q->dirfd);
...
static int consume_ready(void *_arg, _Bool *met)
{
	...
	if((status = get_idx(args->q->dirfd, RD_IDX_NAME, &args->rd_idx)))
		return status;

	uint64_t wr_idx;
	if((status = get_idx(args->q->dirfd, WR_IDX_NAME, &wr_idx)))
		return status;
...
static int produce_ready(void *_arg, _Bool *met)
{
	...
	uint64_t rd_idx;
	if((status = get_idx(args->q->dirfd, RD_IDX_NAME, &rd_idx)))
		return status;

	if((status = get_idx(args->q->dirfd, WR_IDX_NAME, &args->wr_idx)))
		return status;
